...
struct precalculate {
  precalculate(const scoring_function& sf, fl v = max_fl, fl factor_ = 32)
      :  // sf should not be discontinuous, even near cutoff, for the sake of
        ...
        n(sz(factor_ * m_cutoff_sqr) + 3),  // sz(factor * r^2) + 1 <= sz(factor
                                            // * cutoff_sqr) + 2 <= n-1 < n  //
                                            // see assert below
        factor(factor_),
    ...
    VINA_CHECK(sz(m_cutoff_sqr * factor) + 1 < n);  // cutoff_sqr * factor is
                                                    // the largest float we may
                                                    // end up converting into
                                                    // sz, then 1 can be added
                                                    // to the result
