...
#include "third_party/vina/lib/parse_pdbqt.h"
...
void do_randomization(model& m, const std::string& out_name, const vec& corner1,
  ...
  m.write_structure(path(out_name));
}
...
model parse_bundle(const std::string& rigid_name,
  ...
  model tmp = (flex_name_opt) ? parse_receptor_pdbqt(path(rigid_name),
                                                     path(flex_name_opt.get()))
                              : parse_receptor_pdbqt(path(rigid_name));
  VINA_FOR_IN(i, ligand_names)
  tmp.append(parse_ligand_pdbqt(path(ligand_names[i])));
  return tmp;
}

model parse_bundle(const std::vector<std::string>& ligand_names) {
  VINA_CHECK(!ligand_names.empty());  // FIXME check elsewhere
  model tmp = parse_ligand_pdbqt(path(ligand_names[0]));
...
#################################################################\n";
  ...
  try {
    ...
    advanced.add_options()("score_only", bool_switch(&score_only),
        ...
        "repulsion weight")(
        "weight_hydrophobic",
        value<fl>(&weight_hydrophobic)->default_value(weight_hydrophobic),
        "hydrophobic weight")(
        "weight_hydrogen",
        value<fl>(&weight_hydrogen)->default_value(weight_hydrogen),
        "Hydrogen bond weight")(
        "weight_rot", value<fl>(&weight_rot)->default_value(weight_rot),
        "N_rot weight");
    ...
    misc.add_options()("cpu", value<int>(&cpu),
        ...
        "1+")("num_modes", value<int>(&num_modes)->default_value(9),
              "maximum number of binding modes to generate")(
        "energy_range", value<fl>(&energy_range)->default_value(3.0),
        "maximum energy difference between the best binding mode and the worst "
    ...
    weights.push_back(5 * weight_rot / 0.1 - 1);  // linearly maps onto a
                                                  // different range,
                                                  // internally. see
                                                  // everything.cpp
